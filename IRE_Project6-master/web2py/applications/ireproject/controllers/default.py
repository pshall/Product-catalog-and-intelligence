...
def product_details():
    ...
    table.append(tr)
    return dict(table=table, product_id= product_id)

def sell():
    id1=request.args(0)
    record = db(id1 == db.seller.idd).select(db.seller.name,db.seller.price,db.seller.ratings,db.seller.mom)
    l1=len(record)
    if(l1==0):
         session.flash = "no other vendors are available"
    print l1
    return locals()
